...
const headIcon = document.querySelector("#head");
const userName = document.querySelector("#name");
const userRole = document.querySelector("#role");

let loop = 0;

function diologLoad(idx) {
  content.innerText = testimonials[idx].text;
  console.log(headIcon);
  headIcon.src = testimonials[idx].photo;
  userName.innerText = testimonials[idx].name;
  userRole.innerText = testimonials[idx].position;
}

function diologRotate() {
  diologLoad(loop++);
  setInterval(() => {
    if (loop >= testimonials.length) {
      loop = 0;
    }
    diologLoad(loop++);
  }, 4000);
